...
#define INTERNAL_CATCH_TYPE_GEN                                                                                                             \
    ...
    template <template <typename...> class... Cs> constexpr auto get_wrapper() noexcept->TemplateTypeList<Cs...> { return {}; }             \
...
#define INTERNAL_CATCH_NTTP_1(signature, ...)                                                                                                      \
    ...
    template <INTERNAL_CATCH_REMOVE_PARENS(signature)> constexpr auto get_wrapper() noexcept->Nttp<__VA_ARGS__> { return {}; }                     \
...
struct is_callable_tester {
    template <typename Fun, typename... Args> true_given<decltype(std::declval<Fun>()(std::declval<Args>()...))> static test(int);
...
template <typename T> struct is_range_impl<T, typename void_type<decltype(begin(std::declval<T>()))>::type> : std::true_type {
};
} // namespace detail
...
template <typename T, typename AllocComp, typename AllocMatch> struct ContainsMatcher : MatcherBase<std::vector<T, AllocMatch>> {
...
template <typename T, typename AllocComp, typename AllocMatch> struct EqualsMatcher : MatcherBase<std::vector<T, AllocMatch>> {
...
template <typename T, typename AllocComp, typename AllocMatch> struct ApproxMatcher : MatcherBase<std::vector<T, AllocMatch>> {
...
template <typename T, typename AllocComp, typename AllocMatch> struct UnorderedEqualsMatcher : MatcherBase<std::vector<T, AllocMatch>> {
...
template <typename Duration> struct Estimate {
    ...
    template <typename Duration2> operator Estimate<Duration2>() const { return {point, lower_bound, upper_bound, confidence_interval}; }
};
} // namespace Benchmark
} // namespace Catch
...
struct Benchmark {
    ...
    template <typename Fun, typename std::enable_if<!Detail::is_related<Fun, Benchmark>::value, int>::type = 0> Benchmark &operator=(Fun func)
    {
...
template <typename T, bool Destruct> struct ObjectStorage {
    ...
    template <bool AllowManualDestruction = !Destruct> typename std::enable_if<AllowManualDestruction>::type destruct() { stored_object().~T(); }
...
class ExeName : public ComposableParserImpl<ExeName>
{
  ...
  public:
    ...
    template <typename LambdaT> explicit ExeName(LambdaT const &lambda) : ExeName() { m_ref = std::make_shared<BoundLambda<LambdaT>>(lambda); }
