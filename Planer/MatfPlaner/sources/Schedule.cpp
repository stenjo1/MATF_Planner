...
void Schedule::makeSchedule()
{
    ...
    for (int freeDays = 3; freeDays > 0; freeDays--) {
        ...
        //                std::cout << suggestedExams[i]->getSubject().getName().toStdString() << " " <<
        //                suggestedExams[i]->getDate().toString("yyyy.MM.dd").toStdString() << "\n"; std::cout << "[USMENI]" <<
        //                suggestedExams[i]->getSubject().getName().toStdString() << " " <<
        //                suggestedExams[i]->getDateOral().toString("yyyy.MM.dd").toStdString() << "\n";
        //            }
